...
переход на эту комнату, диалог или cutscene.
...
ил container), у объекта, который покидает предмет, вызываются
...
|mp:check\_worn(что)             | Одет предмет? Если да -- попытка снять
