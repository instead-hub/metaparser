...
obj {
	...
	description = function(s)
		p [[Маяк достигает в высоту 100 метров. Его белая башня установлена на небольшом утёсе, омываемым морем с
севера, запада и юга.]];
		if visited 'закат' then
...
obj {
	...
	description = function()
		p [[Высота маяка достигает 100 метров. Белый цвет башни сильно контрастирует с марсианским пейзажем.
...
зажгли свет и яркий луч разрезал сгущающийся мрак.^^
...
room {
	...
	dsc = function(s)
		if s:once() then
			p [[Дверь оказалась не запертой, и ты зашёл внутрь маяка. Неожиданно, Дея схватила тебя за руку и прошептала:^^
-- Когда я скажу "Да, отец!" ты должен закрыть глаза! Запомни!^^
И прежде чем ты успел что-то ответить, она начала подниматься по лестнице, ведущей в маячную комнату.^^]]
			DaemonStop 'девушка'
			move('девушка', 'маячная комната')
		end
		p [[Башня маяка очень высокая. Лестница, закручиваясь в спираль, ведёт наверх. Тебе предстоит преодолеть много ступеней на пути к маячной комнате.]]
	end;
}
...
obj {
	-"стены,стен*";
	found_in = 'в маяке';
	description = [[В стенах нет окон. Свет поступает из светильников, установленных вдоль центральной колонны.]]
}:attr 'scenery'
obj {
	...
	description = [[Спиральная лестница уходит вверх. Если смотреть вверх, это создаёт очень красивую перспективу.]];
}:attr 'scenery'
...
room {
	...
	dsc = function(s)
		if s:once() then
			p [[После долгого подъема, наконец, ты добрался до маячной башни. Здесь тебя уже ждут двое. Кроме
знакомой тебе Деи, здесь есть пожилой мужчина. Судя по всему -- это и есть отец Деи.]]
		end
		p [[Маячная комната представляет собой небольшое круглое помещение с довольно низким потолком. Вдоль всех стен расположены большие прозрачные окна.
В центре находится лестница, которая, вероятно, ведёт к фонарю.]];
	end;
}
obj {
	-"прибор",
...
obj {
	-"старик,отец,мужчина";
	found_in = 'маячная комната';
	description = [[Он невысокого роста, плотного телосложения. Небольшая, но густая борода с сединой, скрывает нижнюю часть его лица.
Старик внимательно смотрит на тебя.]];
	dsc = [[Старик находится рядом с тобой.]];
	each_turn = function(s)
		if s:once() then
			p [[-- А, снова космонавт с Земли -- произносит старик. -- Ну что же, добро пожаловать!]];
		end
	end;
}
