...
obj {
	...
	description = function(s)
		if s:once() then
			p [[Ты осматриваешь стены, пытаясь определить породу необычного камня, как вдруг луч фонаря
выхватывает из темноты череду ровных чёрточек... Ты не веришь своим глазам, но это похоже... На письмена!]]
			enable 'письмена'
			return
		end
		p [[Ты видишь на каменной поверхности письмена, это не может быть правдой! Но ты видишь их. Твое сердце лихорадочно бъётся в груди.]];
	end;
	found_in = 'арка2';
}:attr 'scenery';

obj {
	-"письмена,надписи,чёрточ*";
	nam = "письмена";
	found_in = 'арка2';
	description = [[Не может быть сомнений, что эти надписи оставили разумные существа! Невероятно, как такое возможно?]];
