...
Итак, вы хотите написать парсерный квест. Похвально. Для начала вам можно
прочитать всю документацию STEAD3 и понять общие принципы программирования
на INSTEAD и языке Lua… но тогда бы документация была в разы короче,
а сейчас будет рецепт для нетерпеливых.

Для начала вам необходимо скачать последнюю
версию МЕТАПАРСЕРА (далее МП). Стабильную версию в виде zip архива можно
скачать со [странички INSTEAD3](https://instead-hub.github.io/page/metaparser/).
В архив включены необходимые модули `morph` и `parser`, а также специальная
тема оформления для парсерных игр.
Если вы хотите посмотреть на самую новую, но не совсем отточенную версию,
она выложена в [репозитории git](https://github.com/instead-hub/metaparser).
...
Для того, чтобы создать свою игру, вам нужно создать новый каталог в играх INSTEAD (обычно `Documents and Settings/[ваш юзер]/Local Settings/Application Data/instead/games` или `~/.instead/games`) и скопировать туда модули и тему:
`morph/`, `parser/`, `theme/` и `theme.ini`.

Теперь создайте файл `main3.lua` и сделайте скелет игры:
...
Игра готова. По крайней мере, она готова к запуску. Пройдёмся построчно.

Первая строка - название игры. Это то, что отображается в списке игр
интерпретатора и в заголовке окна.

Вторая строка - перечисление директорий, в которых парсер будет искать слова.
В этом списке должны быть папка с файлом `mp-ru.lua` и все папки, в которых вы
храните файлы игры. Пропускать эту строку можно только если вся ваша игра
записана в файлах `*.lua` в корне игры без подкаталогов. 

`game.dsc` — это то, что выводится в начале игры.
Это не обязательно, можете написать вступление прямо в описании первой локации,
просто к этому мы перейдём позже. Символ `^` обозначает перенос строки.
Про форматирование рассказано в документации INSTEAD, модуль `fmt`.

## Словарь

Метапарсер 3 содержит Большой Словарь Русского Языка (около 8 мегабайт).
Он расположен в файле `morph/morphs.mrd`.
При запуске игры движок смотрит за изменением словарного состава игры и
_компилирует_ маленький словарь для конкретной игры в `dict.mrd`.

Скорость компиляции словаря зависит от процессора и занимает 7-20 секунд.
...
словаря `morph/morphs.mrd`. Это облегчит игру на восемь мегабайт.

## Комнаты и объекты

Давайте добавим комнату в игру. (Далее как пример используется игра «Хейди», см. `games/heidi/main3.lua`)
...
Этот код буквально создаёт комнату с внутренним именем `before_cottage`,
заголовком «Перед домом» и описанием, в котором упоминается избушка и выход
на востоке в лес.
Теперь, чтобы наша игра начиналась с этой локации, а не из комнаты main,
добавим функцию `init`:
...
Что дальше? В описании комнаты есть избушка, значит, игрок может
ей заинтересоваться. Добавляем объект.
...
- определяем имя объекта, по которому его видит игрок;
- определяем факт того, что это словарное слово.
